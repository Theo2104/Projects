...
const solveTolerancePercentage = 0.03;
...
const createPuzzleArray = (rows: number, columns: number, matrix: number[][]) => {
    const puzzleArray: {
      correctPosition: number;
      currentPosXPerc: number;
      currentPosYPerc: number;
    ...
    // Flatten the matrix to get a linear array of positions
    const positions = matrix.flat();
  
    positions.forEach((position, index) => {
      const row = Math.floor(position / columns);
      const col = position % columns;
  
      const currentPosXPerc = col / (columns); // Linear distribution along X axis
      const currentPosYPerc = row / (rows); // Linear distribution along Y axis
  ...
  export const JigsawPuzzle: FC<JigsawPuzzleProps> = ({
    ...
    // Initialize puzzleArray using createPuzzleArray function
    const puzzleArray = useMemo(() => createPuzzleArray(rows, columns, matrix), [rows, columns]);
    ...
    const onImageLoaded = useCallback((image: HTMLImageElement) => {
      ...
      const initialTiles = Array.from(Array(rows * columns).keys()).map(position => ({
        ...
        currentPosXPerc: puzzleArray[position].currentPosXPerc,
        currentPosYPerc: puzzleArray[position].currentPosYPerc,
        solved: false
      }));
  
      setTiles(initialTiles);
    }, [rows, columns, rootSize, puzzleArray]);
